...
from preprocess.extractClick import extract, get_X
import numpy as np
import pickle
...
def get_avg_recommend(models, feature_select_sign, p_threshold, top_threshold, last_day):
    ...
    with open('out/rec_click.txt', 'w') as rec_file:
        for user_id in recommend.keys():
            rec_file.write(user_id + '\t' + ','.join(recommend[user_id]) + '\n')

    count.count_common(last_day=last_day, filename='rec_click.txt')
...
def main():
    ...
    get_avg_recommend(models, feature_select_sign, 0.45, 3000, 91)
...
if __name__ == '__main__':
    ...
    get_avg_recommend(models, feature_select_sign, 0.45, 5000, 122)